...
export function HabitsScreen() {
  ...
  function isCompletedToday(habitId: string) {
    return completions.some(
      (c) => c.habit_id === habitId && isSameDay(new Date(c.completed_at), today),
    );
  }
        ...
        {habits.length === 0 ? (
          <div className="col-span-2 bg-white border border-slate-200/70 rounded-3xl p-10 text-center">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Buat habit baru atau bilang ke AI Chat, "Mau tracking olahraga setiap hari".
            </p>
          </div>
        ) : (
          habits.map((h) => {
              ...
              >
                ...
                <div className="mt-3 flex gap-1">
                  {Array.from({ length: 7 }).map((_, i) => {
                    ...
                    const done7 = completions.some(
                      (c) => c.habit_id === h.id && isSameDay(new Date(c.completed_at), d),
                    );
                    return (
                      <div
                        key={i}
                        className={`size-5 rounded-md text-[9px] grid place-items-center font-medium ${done7 ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400"}`}
                      >
                        {d.getDate()}
                      </div>
                    );
